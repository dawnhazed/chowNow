...
class HistoryFragment : Fragment() {
    ...
    private fun setupRv(){
        ...
        val adapterHistorySaatIni = HistoryOrderAdapter(historyOrderListSaatIni) { order ->
            val intent = Intent(requireContext(), StatusActivity::class.java).apply {
                putExtra("EXTRA_NAMA_RESTO", order.namaResto)
                putExtra("EXTRA_WAKTU_PESAN", order.waktuPesan.time) // Pass as long
                putExtra("EXTRA_JUMLAH_PESANAN", order.jumlahPesanan)
            }
        ...
        val adapterHistorySudahSelesai = HistoryOrderAdapter(historyOrderListSudahSelesai) {click ->
            val intent = Intent(requireContext(), StatusActivity::class.java).apply {
